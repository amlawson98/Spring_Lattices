...
def cubic_lattice(point, number = 4, window_size = 700):
    points = {}
    name = 'Cubic'
    for i in range(0, number):
        for j in range(0, number):
            for k in range(0, number):
                ...
                points[str([i, j, k])] = point(
                    ...
                    neighbors = [
                    str([i, up_neighbor, k]),
                    str([right_neighbor, j, k]),
                    str([left_neighbor, j, k]),
                    str([i, down_neighbor, k]),
                    str([i, j, front_neighbor]),
                    str([i, j, back_neighbor])],
                    lattice_size = window_size,
                    pos = np.array([i, j, k]) * (window_size / number))
    return points, name
...
def make_3d_lattice(point_name_tuple):
    points, lattice_name = point_name_tuple
    for name, point in points.items():
        points[name].pos = np.append(np.array([0.]), points[name].pos)
    return points, name+"_3d_flat"

def uncoupled_pyrochlore_lattice(point, number = 4, window_size = 700):
    name = 'Pyrochlore'
    points = {}
    for l in range(0, number):
        for i in range(0, number):
            for j in range(0, number):
                ...
                if j % 2 == 0:
                    x_pos = i + .5
                    top_right_block = [l, right_neighbor, up_neighbor]
                    top_left_block = [l, i, up_neighbor]
                    left_block = [l, left_neighbor, j]
                    right_block = [l, right_neighbor, j]
                    bottom_left_block = [l, i, down_neighbor]
                    bottom_right_block = [l, right_neighbor, down_neighbor]
                else:
                    x_pos = i 
                    top_right_block = [l, i, up_neighbor]
                    top_left_block = [l, left_neighbor, up_neighbor]
                    left_block = [l, left_neighbor, j]
                    right_block = [l, right_neighbor, j]
                    bottom_left_block = [l, left_neighbor, down_neighbor]
                    bottom_right_block = [l, i, down_neighbor]
                if l % 3 == 0:
                    l_pos_shift = [((8/3)**.5)* ((5/16)**.5)*l, 0, 0]
                if l % 3 == 1:
                    l_pos_shift = [((8/3)**.5)* ((5/16)**.5)*l, 1/2, (5/16)**.5 * (1/3)**.5]
                if l % 3 == 2:
                    l_pos_shift = [((8/3)**.5)* ((5/16)**.5)*l, 0, (5/16)**.5 * 2 * (1/3)**.5]
                for k in range(0, 3):
                    if k == 0:
                        neighbors = [
                        str(right_block + [2]),
                    ...
                    if k == 2:
                        ...
                        str(left_block + [1])]
                        pos_shift = [0, -1/4, -1/2]
                    unmodded_pos = (np.array([0, x_pos, j + 1/2]) + np.array(l_pos_shift) + np.array(pos_shift)) * (window_size / number)
                    modded_pos = unmodded_pos #% window_size
                    points[str([l, i, j, k])] = point(
                        name = str([l, i, j, k]),
                        #magnitude = 400 / number,
                        #direction = random.uniform(0, 2 * pi),
                        neighbors = neighbors,
